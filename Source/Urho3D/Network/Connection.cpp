...
Connection::Payload Connection::Pack(const unsigned char* dataOriginal, unsigned numBytesOriginal)
{
    ...
    const int numBytesCompressed = LZ4_compress_default(
        (char*)dataOriginal, compressedPacketBuffer_.data(), numBytesOriginal, GetMaxPacketSize());
    ...
    URHO3D_ASSERT(payload.numBytes_ <= GetMaxPacketSize());
...
{
    URHO3D_ASSERT(numBytesOriginal <= GetMaxMessageSize());
    URHO3D_ASSERT((dataOriginal == nullptr && numBytesOriginal == 0) || (dataOriginal != nullptr && numBytesOriginal > 0));
    ...
    if (buffer.GetSize() + NetworkMessageHeaderSize + payload.numBytes_ > GetMaxPacketSize())
